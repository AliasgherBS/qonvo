...
_Where we stand, what a machine can check, and what only you can. Run part 1
before every release; run part 2 before anything a customer will touch._
...
1. `./qonvo-up.sh`, then sign in at http://localhost:3002 as the owner.
...
### 1. A provider failure discards the customer's message

**Found 2026-09-04, confirmed on staging. Not yet fixed.**
...
**The fix** is to persist the inbound message and conversation in their own
committed transaction before the LLM is called, leaving only the reply inside the
retried unit. Reproduce it by pointing `QONVO_LLM_API_KEY` at an exhausted key
