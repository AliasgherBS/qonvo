...
|---|---|
| **Secrets remain in git history** | **Updated 2026-09-10: the repository is now PUBLIC**, so the precondition in the original note has passed. Rotation remains the fix that matters, and once a repo has been public a rewrite no longer reliably helps — GitHub keeps unreachable commits addressable by SHA and anyone may already have cloned. Re-counted against `4a48a52`: there are **14** secret-shaped values, not ten, and **13 are rotated**. The exception is `QONVO_MINIO_ACCESS_KEY`, still the exposed value. Low severity (it is the username half, `QONVO_MINIO_SECRET_KEY` was rotated, MinIO binds to `127.0.0.1`, and no MinIO client is constructed anywhere) but it should be rotated. |
